 ...
 */
function detectNegation(message) {
    const negatedTerms = [];
...
function getIntent(message) {
    ...
    // Scenario detection
    if (testRegex(PHRASES.BillingCycleQuestion, msgLower))
        intent.billingCycleQuestion = true;
    if (testRegex(PHRASES.BillingCycleConfusion, msgLower))
        intent.billingCycleConfusion = true;
    if (testRegex(PHRASES.ReplacementScenario, msgLower))
        intent.replacementScenario = true;
    if (testRegex(PHRASES.DiscountQuestion, msgLower))
    ...
    // Topics
    if (/labcorp|lab\s*corp/i.test(msgLower))
        intent.topics.push("labcorp");
    if (/quest/i.test(msgLower))
        intent.topics.push("quest");
    if (/trt|testosterone/i.test(msgLower))
        intent.topics.push("trt");
    if (/hrt|hormone/i.test(msgLower))
        intent.topics.push("hrt");
    if (/glp|weight\s*loss|semaglutide|tirzepatide/i.test(msgLower))
        intent.topics.push("glp");
    if (/insurance/i.test(msgLower))
        intent.topics.push("insurance");
    if (/needle|syringe/i.test(msgLower))
        intent.topics.push("needles");
    if (/travel|traveling|travelling/i.test(msgLower))
        intent.topics.push("travel");
    return intent;
}
function escapeRegex(str) {
    return str.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
function getRelevanceScore(patientMessage, macro) {
    let score = 0;
    const reasons = [];
    ...
    if (asksAboutCharge && !asksAboutRefill) {
        ...
        if (/insurance/i.test(titleClean)) {
            score -= 8.0;
            reasons.push("Insurance irrelevant to charge date");
        }
        if (/refill\s*date|next\s*refill/i.test(titleClean) && !/charge/i.test(titleClean)) {
            score -= 6.0;
            reasons.push("Refill != charge date");
        }
        if (/cancel/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Cancel irrelevant to charge date");
        }
        if (/refund/i.test(titleClean)) {
            score -= 4.0;
            reasons.push("Refund irrelevant to charge date");
        }
        if (/receipt|itemized/i.test(titleClean)) {
            score -= 4.0;
            reasons.push("Receipt irrelevant to charge date");
        }
    }
    // --- INSURANCE QUESTIONS ---
    if (asksAboutInsurance) {
        ...
        if (/charge\s*alignment|charge\s*date/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Charge date irrelevant to insurance");
        }
        if (/cancel/i.test(titleClean)) {
            score -= 4.0;
            reasons.push("Cancel irrelevant to insurance");
        }
    }
    ...
    if (asksAboutCancel) {
        ...
        if (/insurance/i.test(titleClean)) {
            score -= 6.0;
            reasons.push("Insurance irrelevant to cancel");
        }
        if (/charge\s*alignment/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Charge alignment irrelevant to cancel");
        }
    }
    // --- DISCOUNT QUESTIONS ---
    if (asksAboutDiscount) {
        ...
        if (/insurance/i.test(titleClean)) {
            score -= 6.0;
            reasons.push("Insurance irrelevant to discount");
        }
        if (/cancel/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Cancel irrelevant to discount");
        }
        if (/charge\s*alignment/i.test(titleClean)) {
            score -= 4.0;
            reasons.push("Charge alignment irrelevant to discount");
        }
        if (/refill\s*date/i.test(titleClean)) {
            score -= 4.0;
            reasons.push("Refill date irrelevant to discount");
        }
    }
    ...
    if (asksAboutReplacement) {
        ...
        if (/insurance/i.test(titleClean)) {
            score -= 6.0;
            reasons.push("Insurance irrelevant to replacement");
        }
        if (/cancel/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Cancel irrelevant to replacement");
        }
        if (/discount/i.test(titleClean)) {
            score -= 4.0;
            reasons.push("Discount irrelevant to replacement");
        }
    }
    ...
    if (asksAboutLabs && !asksAboutLabBill) {
        ...
        if (/insurance/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Insurance irrelevant to labs");
        }
        if (/cancel/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Cancel irrelevant to labs");
        }
        if (/discount/i.test(titleClean)) {
            score -= 4.0;
            reasons.push("Discount irrelevant to labs");
        }
    }
    // --- LAB BILL QUESTIONS ---
    if (asksAboutLabBill) {
    ...
    if (asksAboutRefund && !asksAboutDiscount) {
        ...
        if (/insurance/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Insurance irrelevant to refund");
        }
        if (/charge\s*alignment/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Charge alignment irrelevant to refund");
        }
        if (/discount/i.test(titleClean)) {
            score -= 4.0;
            reasons.push("Discount irrelevant to refund");
        }
    }
    // --- RECEIPT QUESTIONS ---
    if (asksAboutReceipt) {
        ...
        if (/insurance/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Insurance irrelevant to receipt");
        }
        if (/cancel/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Cancel irrelevant to receipt");
        }
    }
    // --- FSA/HSA QUESTIONS ---
    if (asksAboutFSA) {
    ...
    if (asksAboutShipping && !asksAboutReplacement) {
        ...
        if (/insurance/i.test(titleClean)) {
            score -= 6.0;
            reasons.push("Insurance irrelevant to shipping");
        }
        if (/cancel/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Cancel irrelevant to shipping");
        }
        if (/discount/i.test(titleClean)) {
            score -= 4.0;
            reasons.push("Discount irrelevant to shipping");
        }
    }
    ...
    if (asksAboutNeedles) {
        ...
        if (/insurance/i.test(titleClean)) {
            score -= 6.0;
            reasons.push("Insurance irrelevant to needles");
        }
        if (/cancel/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Cancel irrelevant to needles");
        }
    }
    // --- PRICING QUESTIONS ---
    ...
    if (asksAboutSchedule) {
        ...
        if (/insurance/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Insurance irrelevant to scheduling");
        }
        if (/billing/i.test(titleClean) && !/visit/i.test(titleClean)) {
            score -= 4.0;
            reasons.push("Billing irrelevant to scheduling");
        }
    }
    ...
    if (asksAboutTracking && !asksAboutReplacement) {
        ...
        if (/insurance/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Insurance irrelevant to tracking");
        }
        if (/cancel/i.test(titleClean)) {
            score -= 5.0;
            reasons.push("Cancel irrelevant to tracking");
        }
    }
    // ===========================================================================
...
export function findMacroMatches(patientMessage, macros, topN = 3) {
...
export function getSuggestedResponse(patientMessage, macroMatches) {
    const macrosUsed = [];
